...
st.set_page_config(page_title="Barra Staff V40", page_icon="🍸", layout="wide")
...
# FÓRMULA ANTISCROLL: Calcula altura exacta basada en filas
def calc_altura(df): 
    # Cabecera ~35px + (Filas * 38px) + Borde ~3
    return (len(df) * 38) + 38
...
def run_allocation(event_name):
    ...
    forbidden_map, _, _ = get_forbidden_map(event_name) # Mapa de prohibiciones
    ...
    for barra in ed['Barras']:
        ...
        mat = mat[mat['Nombre'].isin(active)] # Solo habilitados hoy
        team = []
        
        def pick(role_l, role_i, check_col):
            # Candidatos: Tienen el check Y no están asignados hoy
            ...
            for _, r in cands.iterrows():
                ...
                # REGLA DE ORO: Si su barra prohibida es ESTA, se salta.
                if forbidden_map.get(p_clean) == bn:
                    continue 
            ...
            if valid_candidates:
                ...
                # Si todos los candidatos están quemados, SE DEJA VACANTE.
                # NO SE REPITE NUNCA AUTOMATICAMENTE.
...
with t1:
    ...
    if to_del:
        if st.button("🗑️ Eliminar Seleccionados"):
            st.session_state.confirm_del_rh = True
...
with t2:
    ...
    # BORRADO SEGURO EVENTO
    if st.button(f"🗑️ Borrar Evento '{curr_ev}'"):
        st.session_state.confirm_del_ev = True
    if st.session_state.get('confirm_del_ev'):
        ...
        if st.button("⚠️ CONFIRMAR BORRADO EVENTO"):
            ...
            st.session_state.confirm_del_ev = False
            save_data(); st.rerun()
        ...
        for i, l in enumerate(reversed(st.session_state.db_logs)):
            ...
            with st.expander(f"📅 {l['Fecha']} - {l['Evento']}"):
                ...
                if st.session_state.get(f'conf_log_{rx}'):
                    st.warning("¿Seguro?")
                    if st.button("⚠️ SÍ, BORRAR", key=f"cd_{rx}"):
                        st.session_state.db_logs.pop(rx)
                        del st.session_state[f'conf_log_{rx}']
                        save_data(); st.rerun()
